...
func TestGetAllUsersController_Success(t *testing.T) {
	...
	mockUsers := sqlmock.NewRows([]string{"id", "username", "email", "name", "role", "profile_pictur_url", "created_at", "updated_at"}).
	...
	mock.ExpectQuery("SELECT id, username, email, name, role, profile_picture_url, created_at, updated_at FROM users").WillReturnRows(mockUsers)
